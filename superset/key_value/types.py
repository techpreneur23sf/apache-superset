...
# Globals which may be constructed while unpickling a stored value. Containers,
# strings, numbers and booleans are built by the pickle machinery itself and
# never go through ``find_class``, so only the types requiring a global lookup
# are listed here.
PICKLE_ALLOWED_CLASSES: dict[str, frozenset[str]] = {
    "builtins": frozenset(
        {
            "bytearray",
            "complex",
...
class RestrictedUnpickler(pickle.Unpickler):
    """Unpickler which only constructs the value types Superset stores."""

    def find_class(self, module: str, name: str) -> Any:
        if name not in PICKLE_ALLOWED_CLASSES.get(module, frozenset()):
